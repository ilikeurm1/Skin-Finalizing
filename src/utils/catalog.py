...
class CatalogResolver:
    def __init__(self) -> None:
        self.weapon_names = dict(WEAPON_NAMES)
    ...
    def describe_item_details(self, item: InventoryItem) -> str:
        ...
        paint_index = normalize_paint_index(item.attributes.get(PAINT_ATTRIBUTE_ID, ""))
        if pattern_seed:
    ...
    def format_item_label(
        ...
        weapon_name = self.weapon_names.get(
            item.def_index, f"def_index {item.def_index}"
        )
        paint_index = normalize_paint_index(item.attributes.get(PAINT_ATTRIBUTE_ID, ""))
        label = weapon_name
        if paint_index:
            metadata = self.lookup_skin_metadata(item.def_index, paint_index)
            skin_name = self.skin_names.get(
            ...
            if metadata and metadata.phase:
                phase = metadata.phase.strip()
                if phase and phase.lower() not in label.lower():
                    label = f"{label} | {phase}"

            if include_details:
                details = self.describe_item_details(item)
                if details:
    ...
    def is_seed_sensitive(
        ...
        resolved_paint_index = paint_index or normalize_paint_index(
            item.attributes.get(PAINT_ATTRIBUTE_ID, "")
        )
        if not resolved_paint_index:
            return False

        metadata = self.lookup_skin_metadata(item.def_index, resolved_paint_index)
    ...
    def _load_weapon_names(self) -> None:
        data = fetch_json(BASE_WEAPONS_URL)
        ...
        for entry in data:
            if not isinstance(entry, dict):
                continue
            def_index = normalize_scalar(entry.get("def_index"), "")
            name = normalize_scalar(entry.get("name"), "")
    ...
    def _load_skin_names(self) -> None:
        for url in SKIN_API_URLS:
            ...
            for entry in data:
                ...
                name = skin_name_only(normalize_scalar(entry.get("name"), ""))
                image_url = normalize_scalar(entry.get("image"), "")
                if not (paint_index and weapon_id and name):
                    continue

                key = (weapon_id, paint_index)
                ...
                phase = normalize_scalar(entry.get("phase"), "")
                min_float = normalize_float_value(entry.get("min_float"))
                max_float = normalize_float_value(entry.get("max_float"))
                supports_stattrak = bool(entry.get("stattrak"))
                seed_sensitive = is_seed_sensitive_skin(
                    pattern_name=pattern_name,
                    pattern_id=pattern_id,
                    phase=phase,
                )
                existing = self.skin_metadata.get(key)
